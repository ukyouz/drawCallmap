...
class CallSource:
    ...
    def _format_function_call(self) -> str:
        ...
        args = self.get_caller_arguments()
        arg_txt = ("\n" + lineno("") + " " * (len(function_def) + 1)).join(
            ", ".join(args[i : i + 5]) for i in range(0, len(args), 5)
        )
    ...
    def get_caller_arguments(
        ...
        if self.caller.type in {
            ...
            args = set(
                x.text.decode()
                for x in iter_children_by_type(self.argument, "identifier")
            )
            ...
            if only_related:
                continue_args = (set(param_vars.keys()) & args) - IGNORE_ARGS
                if with_type:
                    return [f"{param_vars[var]} {var}" for var in continue_args]
                else:
                    return [f"{var}" for var in continue_args]
            else:
                if with_type:
                    return [f"{param_vars[var]} {var}" for var in param_vars]
                else:
                    return [f"{var}" for var in param_vars]
        else:
            return []
        ...
        for m in matches:
            ...
            if cs:
                ...
                if trace_args and cs.caller.type in {
                    ...
                    continue_args = cs.get_caller_arguments(only_related=True)
...
def write_as_mermaid_flowchart(folder: str, sources: list[CallSource], fp):
    ...
    static_calls = defaultdict(set)
    public_calls = defaultdict(set)
    ...
    for src in call_maps:
        ...
        passed_args = ", ".join(
            src.get_caller_arguments(with_type=False, only_related=True)
        )
        arg_txt = f"-- {passed_args} " if passed_args else ""
        if caller_file == callee_file:
            static_calls[caller_file].add((caller_funcname, src.callee, arg_txt))
        else:
            public_calls[src.filepath].add((caller_funcname, src.callee, arg_txt))
...
def main(search_folder: str, search_term: str, trace_args: bool = True):
    ...
    write_as_mermaid_flowchart(search_folder, sources, diagram_buf)
...
if __name__ == "__main__":
    ...
    try:
        main(args.search_folder, args.search_term, args.trace_args)
    except Exception:
